...
func _create_disciple_data(
	...
	return {
		...
		"realm_id": RealmRegistry.get_id_by_display_name(realm),
		"realm": realm,
		"cultivation": int(extra_data.get("cultivation", extra_data.get("spiritual_power", 0))),
		"at_bottleneck": bool(extra_data.get("at_bottleneck", false)),
...
func export_world_state() -> Dictionary:
	return {
		...
		"territory_states": territory_states.duplicate(true),
		...
		"story_goals": story_goals.duplicate(true),
...
func restore_world_state(state: Dictionary) -> bool:
	...
	territory_states = state.get("territory_states", {}).duplicate(true)
	...
	story_goals = state.get("story_goals", {}).duplicate(true)
